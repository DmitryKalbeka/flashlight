...
import { getProfiler, waitFor } from "@perf-profiler/profiler";
...
export class PerformanceMeasurer {
  ...
  constructor(
    ...
    private options: {
      ...
      adbOptions: AdbPrefixOptions;
    }
  ) {}

  private profiler = getProfiler(this.options.adbOptions);

  private recorder = this.options.recordOptions.record
    ? this.profiler.getScreenRecorder(basename(this.options.recordOptions.videoPath))
    : null;
  ...
  async start(
    ...
    this.polling = this.profiler.pollPerformanceMeasures(this.bundleId, {
